...
!=====end data file setup 
 ...
 do i=1, command_argument_count()
    ...
    select case(adjustl(argv))
     case ("-h", "--help")
           print'(a)', "./PDBcell -d ${data_directory} -n $(skip_step) &
-ib $(init_band_number) -eb $(end_band_number) -w $(wave function)"  
           print'(a)', "-d  : data directory (directory with qm_eigv file)"
           print'(a)', "-n  : how frequently have you printed wave function"
           print'(a)', "-ib : Starting eigen state "           
           print'(a)', "-eb : Ending eigen state "           
           print'(a)', "-w  : Print wave function "
           stop
       case ("-d", "-data")
        call get_command_argument(i+1,argv)
        root_dir=adjustl(argv)
       case ("-n", "--nksip")
          call get_command_argument(i+1,argv); read(argv,*) nskip_step
       case ("-ib", "--init-band")
           call get_command_argument(i+1,argv); read(argv,*) ini_band
       case ("-eb", "--end-band")
           call get_command_argument(i+1,argv); read(argv,*) end_band
       case("-wave","-w")
             getelec=.false.
     case default
    end select
        ...
        if (mod(nstep, nskip_step) /=0) cycle  
            do ii= ini_band, end_band
               !if (mod(ii, nskip_band) /=0) cycle
                ...
                open(10, file=filename2, status='old', action='read', iostat=ios)
                    if (ios/=0) then 
                          print *, ii
                ...
                write(11,'(i4,3f12.6)') nx,HH(1:3,1)/nx
                write(11,'(i4,3f12.6)') ny,HH(1:3,2)/ny
                write(11,'(i4,3f12.6)') nz,HH(1:3,3)/nz
...
species='/md_spc.d'
filename1= trim(root_dir)//trim(species)
open(10, file=filename1, status='old')
      read(10, *)
      read(10, *) ntype, (natomic(it), it = 1, ntype)
end subroutine spc
